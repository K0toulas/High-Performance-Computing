...
int main(int argc, char **argv) {
    ...
    *d_OutputGPU;
    ...
    convolutionRowGPU<<<1, block_dim>>>(d_Buffer, d_Input, d_Filter, imageW, imageH, filter_radius);
    cudaDeviceSynchronize();
    err_runtime = cudaGetLastError();
    CHECK_CUDA_FAIL(err_runtime);
   
    convolutionColumnGPU<<<1, block_dim>>>(d_OutputGPU, d_Buffer, d_Filter, imageW, imageH, filter_radius);
    cudaDeviceSynchronize();
    err_runtime = cudaGetLastError();
    CHECK_CUDA_FAIL(err_runtime);
    
    err_runtime = cudaMemcpy((float *)h_OutputGPU, (float *)d_OutputGPU, imageW * imageH * sizeof(float), cudaMemcpyDeviceToHost);
    CHECK_CUDA_FAIL(err_runtime);

    // Check if GPU output is correct
    // variable to escape outer loop
    int stop = 0; 
    for (i = 0; i < imageW; i++) {
      for (j = 0; j < imageH; j++) {
