  ...
  for (y = filterR; y < imageH+filterR; y++) {
    for (x = filterR; x < imageW+filterR; x++) {
      ...
      for (k = -filterR; k <= filterR; k++) {
        int d = x + k;
        sum += h_Src[y * paddedW + d] * h_Filter[filterR - k];
      }       

      h_Dst[y * paddedW + x] = sum;
    }
  }
...
void convolutionColumnCPU(float *h_Dst, float *h_Src, float *h_Filter,
  ...
  for (y = filterR; y < imageH+filterR; y++) {
    for (x = filterR; x < imageW+filterR; x++) {
      ...
      for (k = -filterR; k <= filterR; k++) {
        int d = y + k;
        sum += h_Src[d * paddedW + x] * h_Filter[filterR - k];
      }   
...
__global__ void convolutionRowGPU(float *d_Dst, float *d_Src, float *d_Filter, 
  ...
  int x, y, k;
  x = threadIdx.x + blockIdx.x * blockDim.x + filterR;
  y = threadIdx.y + blockIdx.y * blockDim.y + filterR;
  int paddedW = 2*filterR + imageW;

  float sum = 0;
  for (k = -filterR; k <= filterR; k++) {
    int d = x + k;     
    sum += d_Src[y * paddedW + d] * d_Filter[filterR - k];
  } 

  d_Dst[y * paddedW + x] = sum;
}
...
__global__ void convolutionColumnGPU(float *d_Dst, float *d_Src, float *d_Filter,
  ...
  for (k = -filterR; k <= filterR; k++) {
    int d = y + k; 
    sum += d_Src[d * paddedW + x] * d_Filter[filterR - k];
  }  
...
int main(int argc, char **argv) {
    ...
    *d_OutputGPU;
    ...
    while(1) {      
      paddedH = paddedW = 2*filter_radius + imageH;
      printf("paddedH %i\n", paddedH);
      stop = 0;
      ...
      h_Input = (float *)calloc(paddedW * paddedH, sizeof(float));
      h_Buffer = (float *)calloc(paddedW * paddedH, sizeof(float));
      h_OutputCPU = (float *)calloc(paddedW * paddedH, sizeof(float));
      h_OutputGPU = (float *)calloc(paddedW * paddedH, sizeof(float));
      ...
      err_Input = cudaMalloc((void **)&d_Input, paddedW * paddedH * sizeof(float));
      err_Buffer = cudaMalloc((void **)&d_Buffer, paddedW * paddedH * sizeof(float));
      err_OuputGPU = cudaMalloc((void **)&d_OutputGPU, paddedW * paddedH * sizeof(float));
      ...
      for (i = filter_radius; i < imageW+filter_radius; i++) {
        for (j = filter_radius; j < imageH+filter_radius; j++) {
          // out side of image boundaries
          index = paddedW*i + j;
          if((i > filter_radius) && (i < (imageW+filter_radius)) \
              && (j > filter_radius) && (j < (imageH+filter_radius))) 
            h_Input[index] = (float)rand() / ((float)RAND_MAX / 255) + (float)rand() / (float)RAND_MAX;
        }
      }
      ...
      printf("GPU computation...\n");
      {
        ...
        err_runtime = cudaMemcpy((float *)d_Input, (float *)h_Input, paddedW * paddedH * sizeof(float), cudaMemcpyHostToDevice);
        CHECK_CUDA_FAIL(err_runtime);
        err_runtime = cudaMemcpy((float *)d_Buffer, (float *)h_Buffer, paddedW * paddedH * sizeof(float), cudaMemcpyHostToDevice);
        CHECK_CUDA_FAIL(err_runtime);
        ...
        cudaMemcpy((float *)h_OutputGPU, (float *)d_OutputGPU, paddedW * paddedH * sizeof(float), cudaMemcpyDeviceToHost);
        
        gpu_time.Stop();
        printf("GPU: Total time = %10g seconds\n\n", gpu_time.Elapsed()/1000);
      }
