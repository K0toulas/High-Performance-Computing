...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i = 1; i < SIZE-1; i+=1) {
        ...
        for (j = 1; j < SIZE-1; j+=1) {
            ...
            p1 = -row_m1[j-1] + row_m1[j+1]
                     - (row0[j-1]<<1) + (row0[j+1]<<1)
                     - row_p1[j-1] + row_p1[j+1];

            p2 = row_m1[j-1] + (row_m1[j]<<1) + row_m1[j+1]
                     - row_p1[j-1] - (row_p1[j]<<1) - row_p1[j+1];
