...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			////// Inline convolution2D + convolution2D Loop unrolling //////
			val = 0;
			p1 = 0;
			p2 = 0;
			
			/* Row i-1 */
			val = input[(i - 1) * SIZE + (j - 1)];
			p1 += val * horiz_operator[0][0]; 
			p2 += val * vert_operator[0][0];

			val = input[(i - 1) * SIZE + j];
			p1 += val * horiz_operator[0][1]; 
			p2 += val * vert_operator[0][1];

			val = input[(i - 1) * SIZE + (j + 1)];
			p1 += val * horiz_operator[0][2]; 
			p2 += val * vert_operator[0][2];

			/* Row i */
			val = input[i * SIZE + (j - 1)];
			p1 += val * horiz_operator[1][0]; 
			p2 += val * vert_operator[1][0];

			val = input[i * SIZE + j];
			p1 += val * horiz_operator[1][1]; 
			p2 += val * vert_operator[1][1];

			val = input[i * SIZE + (j + 1)];
			p1 += val * horiz_operator[1][2]; 
			p2 += val * vert_operator[1][2];

			/* Row i+1 */
			val = input[(i + 1) * SIZE + (j - 1)];
			p1 += val * horiz_operator[2][0]; 
			p2 += val * vert_operator[2][0];

			val = input[(i + 1) * SIZE + j];
			p1 += val * horiz_operator[2][1]; 
			p2 += val * vert_operator[2][1];

			val = input[(i + 1) * SIZE + (j + 1)];
			p1 += val * horiz_operator[2][2]; 
			p2 += val * vert_operator[2][2];
	...
	for (i=1; i<SIZE-1; i++) {
		for ( j=1; j<SIZE-1; j++ ) {
			///// Change pow to simple multiplication //////
			t1 = output[i*SIZE+j] - golden[i*SIZE+j];
			t = t1*t1;
			PSNR += t;
		}
	}
