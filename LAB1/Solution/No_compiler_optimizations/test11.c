...
/* Allocate and fill LUT: sqrt(i) clipped to 255, stored as unsigned char */
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	unsigned char *row_m1, *row0, *row_p1, *out_row, *gold_row;
	int temp;
    ...
    for (i = 1; i < SIZE-1; i++) {
        ...
        temp = i*SIZE;
        out_row = output + temp;
        unsigned char *gold_row = golden + temp;
       	for (j = 1; j < SIZE-1; j++) {
			t1 = *out_row - *gold_row;
			t = t1 * t1;
			PSNR += t;
