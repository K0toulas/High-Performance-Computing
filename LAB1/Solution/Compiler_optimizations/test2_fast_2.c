...
int convolution2D(int posy, int posx, const unsigned char *input, char operator[][3]) {
	...
	// Precompute base addresses for the three rows
	...
	res =
		input[base_y0 + posx - 1] * operator[0][0] +
		input[base_y0 + posx]     * operator[0][1] +
		input[base_y0 + posx + 1] * operator[0][2] +

		input[base_y1 + posx - 1] * operator[1][0] +
		input[base_y1 + posx]     * operator[1][1] +
		input[base_y1 + posx + 1] * operator[1][2] +

		input[base_y2 + posx - 1] * operator[2][0] +
		input[base_y2 + posx]     * operator[2][1] +
		input[base_y2 + posx + 1] * operator[2][2];
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			p1 = convolution2D(i, j, input, horiz_operator);
			p2 = convolution2D(i, j, input, vert_operator);
