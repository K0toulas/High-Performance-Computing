...
// Horizontal Sobel convolution (uses <<1 for *2), precomputes row pointers
int convolution2D_horiz(int posy, int posx, const unsigned char *input) {
   const unsigned char *prev_row = input + (posy - 1) * SIZE;
   const unsigned char *curr_row = input +  posy      * SIZE;
   const unsigned char *next_row = input + (posy + 1) * SIZE;
...
// Vertical Sobel convolution (uses <<1 for *2), precomputes row pointers
int convolution2D_vert(int posy, int posx, const unsigned char *input) {
    const unsigned char *prev_row = input + (posy - 1) * SIZE;
    const unsigned char *next_row = input + (posy + 1) * SIZE;
