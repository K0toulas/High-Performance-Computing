...
/*3.Loop unrolling convolution2D*/
/*4.Common subexpression elimination*/
/*5.Seperate convolution2D in order to simplify multiplications with operators using shift (instead of multiplications with 2)
...
int convolution2D_vert(int posy, int posx, const unsigned char *input);
...
// Horizontal Sobel convolution (uses <<1 for *2)
int convolution2D_horiz(int posy, int posx, const unsigned char *input) {
    int base_y0 = (posy - 1) * SIZE;
    int base_y1 = posy * SIZE;
    int base_y2 = (posy + 1) * SIZE;

    int res = 0;

    // Top row: [-1 0 1]
    res += input[base_y0 + posx + 1];
    res -= input[base_y0 + posx - 1];

    // Middle row: [-2 0 2] → shift instead of multiply by 2
    res += (input[base_y1 + posx + 1] << 1);
    res -= (input[base_y1 + posx - 1] << 1);

    // Bottom row: [-1 0 1]
    res += input[base_y2 + posx + 1];
    res -= input[base_y2 + posx - 1];
...
// Vertical Sobel convolution (uses <<1 for *2)
int convolution2D_vert(int posy, int posx, const unsigned char *input) {
    int base_y0 = (posy - 1) * SIZE;
    int base_y2 = (posy + 1) * SIZE;
...
double sobel(unsigned char *input, unsigned char *output, unsigned char *golden)
{
	...
	for (i=1; i<SIZE-1; i+=1) {
		for (j=1; j<SIZE-1; j+=1 ) {
			...
			p1 = convolution2D_horiz(i, j, input);
            p2 = convolution2D_vert(i, j, input);
